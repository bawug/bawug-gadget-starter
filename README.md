...
5. **Packaging.** Gadgets are first-class ResourceLoader modules. They declare their dependencies in the manifest so the modules resolve before the gadget executes — a user script can pull in the same modules at runtime via `mw.loader.using(...)`, but has to wrap its code in the resulting promise. Gadgets can also bundle multiple files, target specific skins or platforms (`targets=desktop,mobile`), gate by namespace or rights (`rights=patrol`), and act as `package` modules with a `main` entry point. Anything beyond a single self-contained file pushes a user script toward awkward workarounds.
